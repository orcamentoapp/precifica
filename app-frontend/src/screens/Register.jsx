...
export default function Register({ onRegistered, onBackToLogin, initialLicenseCode, lockedEmail }) {
  ...
  async function handleResendLicenseKey() {
    if (!email.trim()) {
      setResendKeyMsg("Preencha o e-mail acima primeiro.");
      return;
    }
  ...
  return (
    <div style={screenStyle}>
      <div style={cardStyle}>
        <AuthLogo />
        <div style={{ fontSize: 20, fontWeight: 800, color: "#0f766e", marginTop: 14 }}>Ativar licença</div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginTop: 6 }}>
            <button
              type="button"
              onClick={handleResendLicenseKey}
              disabled={resendingKey}
              style={{ fontSize: 11.5, fontWeight: 600, color: "#0f766e", background: "none", border: "none", padding: 0, cursor: resendingKey ? "default" : "pointer", textDecoration: "underline" }}
            >
              {resendingKey ? "Reenviando..." : "Não recebeu a chave? Reenviar e-mail"}
            </button>
            {resendKeyMsg && <p style={{ fontSize: 11, color: "#78716c", margin: "4px 0 0" }}>{resendKeyMsg}</p>}
          </div>
