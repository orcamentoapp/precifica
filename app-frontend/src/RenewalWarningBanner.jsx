...
// componente quando nenhum dos dois casos se aplica (ex: licença dada
// manualmente pelo admin). "Renovar agora" manda uma mensagem pro suporte
// pedindo o link de pagamento — mesmo caminho usado em "Gerenciar
// Assinatura" (Perfil), porque o Mercado Pago (que fazia isso sozinho
// antes) foi cancelado, não tem mais nenhum jeito de self-service.
    ...
    try {
      await apiRequest("/api/support/contact", {
        ...
        body: JSON.stringify({
          ...
          message: `Minha licença vence em ${days} ${days === 1 ? "dia" : "dias"} (plano: ${
            plan || "não informado"
          }). Preciso do link de pagamento pra renovar.`,
        }),
      });
      setSent(true);
    } catch (err) {
    ...
    <>
      ...
      >
        ...
        >
          <span>{label}</span>
          {sent ? (
            <span style={{ fontSize: 12, fontWeight: 700 }}>Pedido enviado — aguarde o link por e-mail ✓</span>
          ) : (
            <button
            ...
            >
              {renewing ? "Enviando..." : "Renovar agora"}
            </button>
          )}
        </div>
        {error && (
